...
```sh
"$HOME/.agents/skills/skill-sync/scripts/sync.sh" pull
"$HOME/.agents/skills/skill-sync/scripts/sync.sh" push
```

Do not recreate the synchronization with ad hoc copy or Git commands. The script checks the repository and upstream, requires a clean worktree for pull, and permits push to adopt pending changes only under `.agents/skills` and `.claude/skills`. It refuses unrelated changes or a newer upstream, uses only fast-forward pulls and a plain push, stages only those managed paths, preserves target-only files, and refuses conflicting adapter paths. It never force-pushes or deletes skills.
